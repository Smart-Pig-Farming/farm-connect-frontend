...
export const EditContentWizard = ({
  open,
  onClose,
  onSave,
  content,
}: EditContentWizardProps) => {
  const [draftId] = useState(() => content.id);
  const {
    state: draft,
    setState: setDraft,
    clear: clearDraft,
  } = usePersistentDraft<BestPracticeContentDraft>({
    key: `bp_content_edit_draft_${draftId}`,
  ...
  useEffect(() => {
    if (open) {
      setWizardStep(0);
      // Reset draft to current content when opening
      setDraft({ ...content, updatedAt: Date.now() });
      const t = setTimeout(() => {
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-orange-100 text-sm mt-1 hidden sm:block">
                Update and improve your farming knowledge
              </p>
            </div>
            <button
              onClick={() => {
