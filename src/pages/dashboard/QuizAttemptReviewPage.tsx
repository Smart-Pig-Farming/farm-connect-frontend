...
const getCategoryColors = (color?: string) => {
  const map: Record<
    string,
    { primary: string; accent: string; light: string }
  > = {
    green: {
...
export function QuizAttemptReviewPage() {
  ...
  const totalQuestions =
    reviewData?.attempt?.total_questions || breakdown.length;
      ...
      >
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-lg w-full max-w-md text-center">
          ...
          <p className="text-slate-600 text-sm mb-6">
            We couldn't load this quiz attempt. It may not exist or you may not
            have permission.
          </p>
    ...
    >
      <div className="px-4 sm:px-6 lg:px-8 py-10 max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <button
            onClick={() =>
              navigate(`/dashboard/best-practices/category/${categoryKey}/quiz`)
            }
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-medium transition-colors hover:cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" /> Back
          </button>
          <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-transparent flex-1">
            Quiz Review
          </h1>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3 mb-10">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                <div
                  className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${
                    passed
                      ? `bg-${colors.light} text-${colors.accent}`
              ...
              <div>
                <button
                  onClick={() =>
                    navigate(
                      `/dashboard/best-practices/category/${categoryKey}/quiz/live`
                    )
                  }
                  className={`w-full mt-2 px-4 py-2.5 rounded-xl bg-gradient-to-r ${colors.primary} text-white text-sm font-semibold shadow hover:shadow-md transition-all hover:scale-[1.02] hover:cursor-pointer`}
                >
                  Retake Quiz
                </button>
              </div>
          ...
          <div className="md:col-span-2 flex flex-col gap-6">
            {breakdown.map((b, idx) => {
              ...
              const overlap = selected.filter((id) =>
              ...
              return (
                <div
                  key={b.question_id}
                  className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 border border-white/50 shadow"
                >
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <div className="text-xs font-medium tracking-wide text-slate-500 mb-1">
                        QUESTION {idx + 1}
                      </div>
                  ...
                  <div className="space-y-2 mb-4">
                    {b.options?.map((opt, optIdx) => {
                      ...
                      if (isOptCorrect && chosen)
                        style = "bg-green-50 border-green-500 text-green-800";
                      else if (isOptCorrect && !chosen)
                        style =
                          "bg-green-50/60 border-green-300 text-green-700";
                      else if (!isOptCorrect && chosen)
                        style = "bg-red-50 border-red-300 text-red-700";
                      return (
                        <div
                          key={opt.id}
                          className={`rounded-xl border px-4 py-2 flex items-start gap-3 ${style}`}
                        >
                          <div className="pt-0.5 text-xs font-mono text-slate-400">
                            {String.fromCharCode(65 + (optIdx || 0))}
                          </div>
                          ...
                          {isOptCorrect && !chosen && (
                            <CheckCircle2 className="w-4 h-4 text-green-400 shrink-0 opacity-70" />
                          )}
                          {!isOptCorrect && chosen && (
                            <XCircle className="w-4 h-4 text-red-500 shrink-0" />
                          )}
                          {isOptCorrect && !chosen && (
                            <span className="ml-2 text-[10px] font-semibold uppercase tracking-wide text-green-600 bg-white/70 border border-green-200 px-1.5 py-0.5 rounded">
                              Answer
                            </span>
                          )}
                          {!isOptCorrect && chosen && (
                            <span className="ml-2 text-[10px] font-semibold uppercase tracking-wide text-red-600 bg-white/70 border border-red-200 px-1.5 py-0.5 rounded">
                              Your Choice
                            </span>
                          )}
                        </div>
